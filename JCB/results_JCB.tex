...
were $75\text{bps}$ long for an average fold coverage of $5$, $10$ and $15$. 
 They yield respectively an average error rate of $2.4\%$, $0.9\%$ and $0.6\%$. \ART reads
 are automatically mapped to the sequence of interest. For every position covered by reads, 
 we selected the nucleotide appearing more frequently. At un-covered position, we picked a random
 nucleotide.
...
We used a value of $\beta=15$ and evaluated $\alpha$ at $0, 0.5, 0.8, 1.0$. 
 ...
 Due to the low number of errors, we set the presumed as twice the average error rate made by \ART, i.e. $4.8\%$, $1.8\%$ and $1.2\%$ for fold coverages of $5$, $10$ and $15$ respectively. \ART reads
...
To evaluate our method, we simulated with a random sequences from the MSA every time,
 ...
 evaluated the ROC curve mostly as in Sec.~\ref{sec:5S}. In Fig.~\ref{fig:16s_all} they are computed
 over all positions, while in Fig.~\ref{fig:16s_bp} only over positions inside a base pair.
...
contributions to the energy only comes from structured regions, this was an expected result.
...
in that case best performances were observed for non-nil values of $\alpha$.
