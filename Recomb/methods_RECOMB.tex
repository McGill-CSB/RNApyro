...
can be decomposed as a sequence preceded by a, possibly mutated, base at $i$ 
(Unpaired case), a sequence surrounded by some base-pair (Stacking-pair case), 
or as two sequences separated by some base-pair (General base-pairing case). 
...
equal to $\Z{j,n-1}{m}{X,X}$, for any $X\in B$. The recursion itself unfolds as follows.
\begin{equation}
\resizebox{\textwidth}{!}{%
	$\Y{i,j}{m}{a,b} = \left\{
  \begin{array}{ll}
