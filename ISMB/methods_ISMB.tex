...
More precisely, let us consider the following predicate: {\em Let $(i',j')$ be the base-pair such that $i'<i<j'$ while minimizing $i-i'$, then TODO!!!!
...
Once the partition functions have been computed, a stochastic backtrack starts from the complete interval $[0,|\Target|-1]$ and, at each step, chooses a suitable assignment for one or several positions, as described in Algorithm~\ref{alg:back}. 
\begin{algorithm}[t]
	\SetAlgoLined
\SetKwFunction{Backtrack}{Backtrack}
\SetKwFunction{Random}{Random}
	rand $\leftarrow$ \Random$\left(\Z{i,j}{a,b}\right)$\;
	k $\leftarrow S_i$\;
	\If(\tcp*[f]{Unpaired}){$k = -1$}{
		\For{$a'\in\B$}{
			rand $\leftarrow$ rand $- x^{\gc(a')}\cdot \Z{i+1,j}{a',b}$\;
			\lIf{$\text{rand}<0$}\Return{$a'.\Backtrack\left((i+1,j),(a',b),S\right)$}\;
		}
	}
	\Else{
		\If(\tcp*[f]{Stacking pairs}){$k=j \land S_{i-1}=j+1$}{
			\For{$(a',b')\in\B\times\B$}{
				rand $\leftarrow$ rand $-
			 	...
			 	\cdot \Z{i+1,j-1}{a',b'}	$\;
				\If{$\text{rand}<0$}{
					\Return{$a'.\Backtrack\left((i+1,j-1),(a',b'),S\right). b'$}\;		
				}
			}
		}
		\Else{
       ...
       x^{\gc(a'.b')}
	    	  ...
	    	  \cdot\Z{k+1,j}{b',b}$\;	
 					\If{$\text{rand}<0$}{
						\Return{$a'
						.\Backtrack\left((i+1,k-1),(a',b'),S\right)
						.b'
						.\Backtrack\left((k+1,j),(b',b),S\right)
						$}\;	
					}
				}
		 }
    \Else {\Return{$\varepsilon$};	\tcp*[f]{Empty sequence}}
		}
	}
\caption{\protect\Backtrack$\left((i,j),(a,b),S\right)$\label{alg:back}}
\end{algorithm}
