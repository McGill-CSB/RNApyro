...
%Such terms can be defined by rec.urrence, for which the following initial conditions holds:
%
...
\begin{algorithm}
	\SetAlgoLined
	\lIf{$i>j$} \Return{$\varepsilon$};\tcp*[f]{Empty sequence}\\
	rand $\leftarrow$ Random$\left(\Z{i,j}{a,b}\right)$\;
	k $\leftarrow S_i$\;
	\If(\tcp*[f]{Unpaired}){$k = -1$}{
		\For{$a'\in\B$}{
			rand $\leftarrow$ rand $- \Prob(a')\cdot \Z{i+1,j}{a',b}$\;
			\lIf{$\text{rand}<0$}\Return{$a'\cdot\text{Backtrack}\left((i+1,j),(a',b),S\right)$}\;
		}
	}
	\Else{
		\If(\tcp*[f]{Stack}){$k=j \land S_{i-1}=j+1$}{
			\For{$(a',b')\in\B\times\B$}{
				rand $\leftarrow$ rand $-
			 	\Prob(a')
			 	\cdot\Prob(b')
			 	...
			 	\cdot \Z{i+1,j-1}{a',b'}	$\;
				\If{$\text{rand}<0$}{
					\Return{$a'\cdot\text{Backtrack}\left((i+1,j-1),(a',b'),S\right)\cdot b'$}\;		
				}
			}
		}
		\Else(\tcp*[f]{Paired not stacked}){
			\For{$(a',b')\in\B\times\B$}{
				rand $\leftarrow$ rand $-
				\Prob(a')
				\cdot\Prob(b')
    	  ...
    	  \cdot\Z{k+1,j}{b',b}$\;	
 				\If{$\text{rand}<0$}{
					\Return{$a'
					\cdot\text{Backtrack}\left((i+1,k-1),(a',b'),S\right)
					\cdot b'
					\cdot\text{Backtrack}\left((k+1,j),(b',b),S\right)
					$}\;	
				}
			}
		}
	}
\caption{Backtrack$\left((i,j),(a,b),S\right)$\label{alg:back}}
\end{algorithm}
