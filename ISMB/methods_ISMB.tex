...
Due to the simplification in our energy model, unpaired regions are not subject design constraints other than the \GCContent, leading to modest similarities of refolded design candidates with the targeted structure. To improve these performances and test the complementarity  of our global sampling approach with previous contributions based on local search, we used the \RNAinverse software to redesign unpaired regions, specifying a constraint mask to prevent stacking base-pairs from being modified.
